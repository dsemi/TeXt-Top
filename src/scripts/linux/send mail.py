    ...
    if not len(file_name)==0:
        ...
        for i in file_name:
            try:
                ...
                p = Popen(shlex.split('./find\ file.sh %s' % (i)), stdout=PIPE)
                ...
                #attach it
                part.add_header('Content-Disposition', 'attachment; filename='+re.match('.*/(.*)',filename).group(1))
                msg.attach(part)

            except IOError:
                print "The file: "+i+' was not found.\n'
